...
const addLabelledField = (
  scene: Phaser.Scene,
  container: Phaser.GameObjects.Container,
  labelText: string,
  fieldText$: Observable<string>,
  offsetY: number = 10
) => {
  const label = new Phaser.GameObjects.Text(scene, 0, offsetY, labelText, {});
  const field = new Phaser.GameObjects.Text(
    scene,
    0,
...
export const controlsPanelView = (
  scene: Phaser.Scene,
  ...
  container.add(
    new Phaser.GameObjects.Rectangle(
      ...
      .setStrokeStyle(1, panelBorderColour)
      .setOrigin(0, 0)
  );
  ...
  addLabelledField(
    ...
    container,
    "atan2",
    sharedController.angleFromSelectedTile$.pipe(mapToString)
  );
};
